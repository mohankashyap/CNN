...
from grcnn import GrCNN, GrCNNMatchScorer, ExtGrCNNMatchScorer
...
config_filename = './extgrcnn.conf'
start_time = time.time()
configer = GrCNNConfiger(config_filename)
grcnn = ExtGrCNNMatchScorer(configer, verbose=True)
end_time = time.time()
logger.debug('Time used to build/load ExtGrCNNMatchRanker: %f seconds.' % (end_time-start_time))
...
fudge_factor = 1e-6
logger.debug('ExtGrCNNMatchRanker.params: {}'.format(grcnn.params))
...
sio.savemat('extgrcnn_ranker_initial.mat', initial_params)
